...
// var
var (
	errPort = errors.New(`serial configuration error:
		invalid port (unix: /dev/tty* , windows: COM*)`)
	errBaud = errors.New(`serial configuration error:
		invalid baud rate (4800, 9600, 19200, 38400, 57600, 115200)`)
	errDataBit = errors.New(`serial configuration error:
		invalid number of data bits (5, 6, 7, 8, 9)`)
	errStopBit = errors.New(`serial configuration error:
		invalid number of stop bits (1, 2)`)
	errParity = errors.New(`serial configuration error:
		invalid parity (0 - None, 1 - Even, 2 - Odd)`)
	errConnOpen = errors.New(`serial connection error:
		connection is not open`)
	errWriteBytes = errors.New(`serial write error:
		byte count for transmission is mismatching`)
)
...
// LoadConnection provides a connection with the parameters being loaded
// from a json file.
